...
세상 속에서 우리가 알고자 하는 것들($Y$)은 여러가지 요인들($X_1, X_2, ..., X_p$)에 의해 설명될 수 있는 부분과, 그렇지 못하는 부분($\epsilon$, 이때 $\epsilon$은 $X$와 독립이며 평균이 $0$인 확률변수)의 합이라고 볼 수 있다. 따라서 우리는 모든 현상을 어느정도 설명할 수 있지만, 완벽히 설명하지는 못한다. 우리 주변의 현상들이 설명 가능할 때 우리는 많은 가치를 창출할 수 있다. 그리고 Statistical Learning은 설명 가능한 부분인 $f$를 추정하기 위한 접근법이다.
...
### 1.1.3. The Trade-Off Between Prediction Accuracy and Model Interpretability

![FIGURE 2.7](/ISLR/FIGURE_2.7.png)

모델의 예측 정확도와 해석력은 Trade-Off 관계에 놓여있다. 따라서 해석이 목표인 경우에는 상대적으로 덜 유연한 모형을, 예측이 목표인 경우에는 유연한 모형을 사용하는 것이 좋다.

### 1.1.4. Supervised Versus Unsupervised Learning
...
### 1.1.5. Regression Versus Classification Problems
...
![FIGURE 2.9](/ISLR/FIGURE_2.9.png)
...
![FIGURE 2.14](/ISLR/FIGURE_2.14.png)
